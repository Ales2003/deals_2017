...
//@Repository
public class OldCharacterTypeDaoImpl implements ICharacterTypeDao {

	private static final Logger LOGGER = LoggerFactory.getLogger(OldCharacterTypeDaoImpl.class);
	...
	public List<CharacterType> getAll() {
		try {
			...
			List<CharacterType> characterTypes = jdbcTemplate.query(READ_ALL_SQL,
					new BeanPropertyRowMapper<CharacterType>(CharacterType.class));
			LOGGER.debug("[{}]. Store returns [{}] entitys.", CharacterType.class.getSimpleName(),
					characterTypes.size());
			return characterTypes;
		} catch (EmptyResultDataAccessException e) {
			String errMsg = String.format("Class [%s]. Storage returns incorrect entity count.",
					CharacterType.class.getSimpleName());
			LOGGER.error("Error: {}", errMsg);
			throw e;
		}
	}
